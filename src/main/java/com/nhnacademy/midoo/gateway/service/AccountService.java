...
import com.nhnacademy.midoo.gateway.util.RestTemplateUtil;
...
public class AccountService {
    ...
    public String createAccount(AccountDto dto) throws JsonProcessingException {
        ...
        String result = RestTemplateUtil.createQuery(template, accountUrl, accountPort, "/accounts", HttpMethod.POST,
                mapper.writeValueAsString(dto), String.class).getBody();
        return result;
    }
...
//      return "no_data";
//    }
        return template.getForObject(accountApiServerProperties.getFullUrl() + "/accounts/{memberId}/detail",
                String.class, id);
    }
...
//      return "no_data";
//    }
        return template.getForObject(accountApiServerProperties.getFullUrl() + "/accounts/{memberId}", String.class,
                id);
    }
...
//    }
    ...
    public ResponseEntity<Boolean> checkExistId(String id) {
        ResponseEntity<Boolean> result = template.postForEntity(accountApiFullUrl + "/accounts/exist/id",
                new HttpEntity<Map<String, String>>(Map.of("clientId", id), null), Boolean.class);
    ...
    public ResponseEntity<Boolean> checkExistEmail(String email) {
        ResponseEntity<Boolean> result = template.postForEntity(accountApiFullUrl + "/accounts/exist/email",
                new HttpEntity<Map<String, String>>(Map.of("clientEmail", email), null), Boolean.class);
        if (result.getStatusCodeValue() != 200) {
            log.info("status code check {}", result.getStatusCodeValue());
            return ResponseEntity.ok(false);
        }
    ...
    public String getPwdById(String memberId) {
        return template.getForObject(accountApiServerProperties.getFullUrl() + "/accounts/{memberId}/password",
                String.class, memberId);
    }

    public String getEmailById(String memberId) {
        return template.getForObject(accountApiServerProperties.getFullUrl() + "/accounts/{memberId}/email",
                String.class, memberId);
    }

    public void putAccount(AccountDto dto, String memberId) {
        template.postForObject(accountApiServerProperties.getFullUrl() + "/accounts/{memberId}/update", dto,
                String.class, memberId);
    }
